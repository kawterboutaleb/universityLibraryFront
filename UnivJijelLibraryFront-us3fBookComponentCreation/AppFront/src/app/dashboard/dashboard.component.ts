...
private createChartPie2(): void {
  ...
  let nbr_loans:number[]=[5,6,10,2,15,8,9,7,3,2,5,9];
 ...
 let counter=4;
 const nbr_months=months.length;
  setInterval(() => {
    //date.setDate(date.getDate() + 1);
    counter=(counter+1)%nbr_months;
    chart.series[0].addPoint({
      name:months[counter],//`${date.getDate()}/${date.getMonth() + 1}`,
      y: nbr_loans[counter]//, this.getRandomNumber(0, 1000),
    }, true, true);
  }, 2500);
}
